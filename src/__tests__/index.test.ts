...
describe("SupabaseDistributor", () => {
  ...
  test("should throw error when initializing without config", async () => {
    await expect(distributor.initialize(mockFeedId, {})).rejects.toThrow(
      "Missing required config: supabaseUrl and supabaseKey",
    );
  });

  test("should throw error when distributing without initialization", async () => {
    await expect(
      distributor.distribute(mockFeedId, mockContent),
